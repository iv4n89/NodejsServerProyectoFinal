    ...
    const Films = sequelize.define('Films', {
        ...
        puntuacion_media: {
            ...
            get: async function () {
                ...
                // const total = await sequelize.query(`SELECT FilmId, COUNT(id) as comentarios FROM comments WHERE FilmId = ${this.id}`, { type: QueryTypes.SELECT });
                console.log(sum.total, sum.comentarios);
                // console.log(total);
                const media = Number(sum.total) / Number(sum.comentarios);
                console.log(media);
                return sum;
            }
        }
