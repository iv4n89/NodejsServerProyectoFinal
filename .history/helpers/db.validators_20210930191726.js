...
const userHasCommentOnFilm = async (id) => {
    const existeComentario = await Comments.findOne({ where: { UserId: id } });
    if (existeComentario) {
        throw new Error('The user already has commented the film');
    }
}
