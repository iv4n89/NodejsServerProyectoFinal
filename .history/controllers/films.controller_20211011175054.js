...
const allFilmsGet = (req, res) => {
    const { limit = 5, offset = 0, contains = "", genero = "", year = 0 } = req.query;
    const options = { titulo: { [Op.substring]: contains }, estreno: { [Op.gte]: year }, genero: [Op.substring]: genero };
    Promise.all([
...
const newFilmPost = (req, res) => {
    const { titulo, estreno, img, genero } = req.body;
    const { id: UserId } = req.user;
    Films.create({ titulo, estreno, img, UserId, genero })
        .then(result => {
